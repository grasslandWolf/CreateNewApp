                   ...
                   type:(FVAlertType)type
{
    ...
    CGRect wFrame = view.frame;
    CGFloat h = wFrame.size.height;
    CGFloat w = wFrame.size.width;
    //get window size and position
    CGRect windowRect = CGRectMake(0, 0, h>w?h:w, h<w?h:w);

    //create the final view with a special tag
    UIView *resultView = [[UIView alloc] initWithFrame:wFrame];
    ...
    content.center = CGPointMake(alertView.frame.size.width / 2, alertView.frame.size.height / 2);
...
+ (void)showSmallLoadingAlertOnView:(UIView *)view withTitle:(NSString *)title {
...
+ (void)hideAlertByTap:(UITapGestureRecognizer *)sender {
    ...
    [self fadeOutView:sender.view
           completion:^(BOOL finished) {
             ...
             //  [[NSNotificationCenter defaultCenter] postNotificationName:notiDidHideAlertViewByTap object:nil];
           }];
}
